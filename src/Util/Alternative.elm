module Util.Alternative exposing ( ContentWidth
                          , Factor
                          , Milliseconds
                          , Offset
                          , Label
...
-- simple calc
testStart = 1325376000000
testEnd = 1325793600000
cWidth = 1580
conver = calcConversion testStart testEnd cWidth 
         
minTimeStep cw c =
    displayToMs (cw * 6) c - Tuple.first c
...
mst = minTimeStep 7 conver

calcStep v =
    if v > stepYear * 40
    then
        let
            minor = round <| toFloat v / toFloat stepYear
        in
            (Year 1, Year minor)
    else if stepYear > v && stepMonth < v
    then
        let
            minor = round <| toFloat v / toFloat stepMonth
            major = round <| toFloat stepYear / toFloat (stepMonth * minor)
        in
            (Year major, Month minor)
    else if stepMonth > v && stepDay < v
    then
        let
            minor = round <| toFloat v / toFloat stepDay
            major = round <| toFloat stepMonth / toFloat (stepDay * minor)
        in
            (Month major, Day minor)
    else if stepDay > v && stepHour < v
    then
        let
            minor = round <| toFloat v / toFloat stepHour
            major = round <| toFloat stepDay / toFloat (stepHour * minor)
        in
            (Day major, Hour minor)
    else if stepHour > v && stepMinute < v
    then
        let
            minor = round <| toFloat v / toFloat stepMinute
            major = round <| toFloat stepHour / toFloat (stepMinute * minor)
        in
            (Hour major, Minute minor)
    else if stepMinute > v && stepSecond < v
    then
        let
            minor = round <| toFloat v / toFloat stepSecond
            major = round <| toFloat stepMinute / toFloat (stepSecond * minor)
        in
            (Minute major, Second minor)
    else if stepSecond > v && stepMillisecond < v
    then
        let
            minor = round <| toFloat v / toFloat stepMillisecond
            major = round <| toFloat stepSecond / toFloat (stepMillisecond * minor)
        in
            (Second major, Millisecond minor)
    else (Second 10, Millisecond 1)
...
-- stepDay, stepHour
-- stepHour, stepMinute  
-- stepMinute, stepSecond
-- stepSecond, stepMillisecond
-- findStep mst

-- round <| toFloat mst / toFloat stepHour
...
minimumStep : Float -> Conversion -> Milliseconds
minimumStep w c =
...
displayToMs : Float -> ( Offset, Factor ) -> Milliseconds
displayToMs v ( o, f ) =
  round <| v / f + toFloat o
...
setMinimumStep : Milliseconds -> Scale
setMinimumStep m =
